      ...
      subroutine abpop
!******************************************************************************
!     Equivalent width matching routine for stellar populations; usually
!     meant for integrated light spectra.
!******************************************************************************
...
!*****open the model table input file and the summary table output file;
!     read the information from the table input file
      call tablepop (1)
...
!*****FIRST PASS:  For each model, for each line, create a curve-of-growth
!     finely divided in steps of 0.01 in "Ngf"
      do mmod=1,modtot


!*****read in the model atmospheres and their summary output files
         line = synpre
         num = 80
...
!*****open and read the line list; this will internally be broken down
!     into 1-line lists for computational efficiency
         array = 'THE LINE LIST'
         nchars = 13
         nflines = 31
         lscreen = 14
...
!*****do the curves of growth; store the results
         rwlow = -6.5
         rwhigh = -3.5
         rwstep = 0.15
         do lim1=1,nlines
...
!*****now, inside the
!*****define the range of lines for a species
 5       call linlimit
...
!*****for just the first model atmosphere, interpolate in the curve-of-growth
!     at very small log(Ngf) steps for just the first line in the list,
!     in order to make a lookup table for later abundance iterations
...
!*****now consider each line at a time; use these curve-of-growth to
!     compute a weighted <EW_calc>
...
!*****here we go for a final iteration on a line, or finish
         call ewweighted
      enddo

      
!*****do the species statistics and output the results
      call stats
      call lineinfo (3)
