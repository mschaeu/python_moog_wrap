      ...
      subroutine abunplot
!******************************************************************************
!     This routine produces MONGO plots of line abundances versus
!     excitation potentials and equivalent widths
!******************************************************************************
...
!*****dump the data into working arrays
      j = 0
      do l=lim1obs,lim2obs
         if (abundout(l) .ne. 999.99) then
            ...
            ep(j) = e(l,1)
            abb(j) = abundout(l)
!           logrw(j) = dlog10(wid1comp(l)/wave1(l))
...
!*****find the plot boundaries for the excitation potential plot
      xlo = 20.
      do j=1,kount
         xlo = amin1(xlo,ep(j))
      enddo
...
!*****start the excitation potential plot via plot setup calls
...
!*****draw and label the box for the excitation potential plot
      call defcolor (1)
      call sm_lweight (4.0)
      call sm_expand (1.2)
      call sm_box (0,0,0,0)
...
!*****make the excitation potential plot
      call defcolor (2)
      call sm_expand (2.2)
      style(1) = 43.5
...
!*****define the plot limits for the equivalent width plot
      xlo = 5000.
      do j=1,kount
...
!*****draw and label the box for the equivalent width plot
      call defcolor (1)
      call sm_lweight (4.0)
      call sm_expand (1.2)
      call sm_box (0,0,0,0)
...
!*****make the equivalent width plot
      call defcolor (2)
      call sm_expand (2.2)
      style(1) = 43.5
      call sm_ptype (style,1)
...
!*****define the plot limits for the wavelength plot
      xlo = 5000000.
      do j=1,kount
...
!*****draw and label the box for the wavelength plot
      call defcolor (1)
      call sm_lweight (4.0)
      call sm_expand (1.2)
      call sm_box (0,0,0,0)
...
!*****make the wavelength plot, and exit normally
      call defcolor (2)
      call sm_expand (2.2)
      style(1) = 43.5 
      call sm_ptype (style,1)
