      ...
      subroutine begin
!***************************************************************************
!    This routine simply starts up MOOG
!    THIS VERSION IS FOR LINUX REDHAT MACHINES
!***************************************************************************
...
!*****define the number of text screen lines for silent mode;
!    this number is hardwired, since it is not really needed at run time.
      if (silent .eq. 'y') then
         maxline = 24
         go to 10
      endif
...
!*****define the number of lines available on the text screen for
!    interactive mode; this number is discovered from the "stty -a"
!    command, for which the output format is unique to the operating
!    system.
...
!*****open data files carried with the source code: Barklem damping
      nfbarklem = 35
      num = 60
...
!*****open data files carried with the source code: Barklem UV damping
      nfbarklemUV = 36
      num = 60
      call getcount (num,moogpath)
      if (moogpath(num:num) .ne. '/') then
...
! write a header and find the appropriate parameter file, and exit normally
