...
!******************************************************************************
!     This routine concatenates two syntheses in order to make a smoothed
!     binary synthetic spectrum
!******************************************************************************
...
!*****compute and output the parameters related to the spectrum combination:
!     the declared velocity difference, the luminosity ratio, and
!     for information, the computed fluxes of the two stars; get the user's
!     agreement
...
!*****read back the header information from the individual star raw
!     synthetic spectra
...
!*****read back the raw synthetic spectra from the individual stars;
!     shift by the appropriate point number, add the spectra (normalized),
!     and dump to combined raw synthetic spectrum file
      read (nf7out,1006) (ysyn(i),i=1,kount)
      read (nf8out,1006) (zsyn(i),i=1,kount)
      do i=1,kount
         ysyn(i) = 1.0 - ysyn(i)
         zsyn(i) = (1.0 - zsyn(i))/lumratio
      enddo
      ...
      if (pshift .gt. 0) then
         ...
         enddo
         do i=pshift+1,kount
            dev(i) = (ysyn(i)+zsyn(i-pshift))/normalization
         enddo
      elseif (pshift .eq. 0) then
         do i=1,kount
            dev(i) = (ysyn(i)+zsyn(i))/normalization
         enddo
      else
         do i=1, kount-pshift
            dev(i) = (ysyn(i)+zsyn(i-pshift))/normalization
      ...
      do i=1,kount
         ysyn(i) = 1.0 - dev(i)
      enddo
      write (nf9out,1006) (ysyn(i),i=1,kount)
      go to 99
100   return
