      ...
      subroutine blends
!******************************************************************************
!     This program does abundance derivations from blended spectral features;
!     only one element will have its abundance derived per run.
!******************************************************************************
...
!*****open the files for standard output and summary abundances
      nf1out = 20
      lscreen = 4
...
!*****start the large loop that will go through each blended feature
      ewsynthopt = -1
      mode = 4
30    do lll=1,1000
...
!*****define the set of lines responsible for a blended feature
         call linlimit
         lim1 = lim1line
         lim2 = lim2line
...
!*****make sure that the element whose abundance is to be fit has
!     a representative line of the blend
         ifind = 0
         do j=lim1,lim2
...
!*****do the syntheses, forcing each abundance to predict the
!     feature equivalent width; a limit of 30 iterations is imposed
!     arbitrarily
         ncurve = 1
...
!*****here we go for another iteration
...
!*****here we do the final calculation when the predicted and observed are close
         gf1(ncurve) = gf1(ncurve-1)*ratio
         do j=lim1,lim2
            if (atom1(j) .gt. 100.) then
               call sunder (atom1(j),ia,ib)
...
!*****here is where some auxiliary things like mean depth are computed
...
!*****assign the abundance to the strongest line of the blend that
!     contains cogatom; put 999.99's as the abundances of all but
!     this line; go back for another blended feature
...
!*****do abundance statistics; print out a summary of the abundances
      lim1obs = 1
...
!*****now the option will be to redo the molecular equilibrium and redo
!     the last species, at the user's option.
      if (neq .ne. 0) then
         do n=1,neq
