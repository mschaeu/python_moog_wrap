...
!     H, He, C, N, O, Mg, Al, Si, P, S, Cl, Ca, Ti, V, Cr, and Fe. The 
!     molecules in this list were selected based on predicted partial pressures
!     for T > 3000 K. The cutoff was 1e-7 percent of the total gas pressure. 
!     The user is encouraged to check the completeness of this list for the 
!     physical conditions under consideration.
!
!     This routine also contains the molecular equilibrium constant data for 
!     the most common diatomic and triatomic molecules. The equilibrium 
!     constants were calculated from the parital pressures in the Phoenix 
!     (NextGen) equation of state table on June 27, 2005. The data below are 
!     the coefficients for polynomial fits to the constants and span the range 
!     1500 < T < 10000 K. The format is: molecule id, Do, coef(1:5) 
!     The constants are reconstructed by a polynomial expansion:
!     log10(Kp) = sum{i=1:5}{coef(i)*log10(theta)**(i-1)} - Do*theta
!******************************************************************************
      implicit real*8 (a-h,o-z)
      include 'Mol.com'


!*****Here is the default small list of ions and molecules.
!     P(H)  = p(H) + p(H+) + 2p(H_2) + p(CH) + p(NH) + p(OH) + p(MgH) + 2p(H_2O)
!     P(C)  = p(C) + p(C+) + p(CH) + p(C_2) + p(CN) + p(CO) + p(CO_2)
!     P(N)  = p(N) + p(N+) + p(NH) + p(CN) + 2p(N_2) + p(NO)
!     P(O)  = p(O) + p(O+) + p(OH) + p(CO) + p(ON) + 2p(O_2) + 2p(CO_2)
!     P(Mg) = p(Mg) + p(Mg+) + p(MgH) + p(MgO)
!     P(Ti) = p(Ti) + p(Ti+) + p(TiO)
...
!*****Here is the default large list of ions and molecules.
!     P(H)  = p(H) + p(H+) + 2p(H_2) + p(CH) + p(NH) + p(OH) + p(MgH) + p(AlH)
!                  + p(SiH) + p(PH) + p(SH) + p(ClH) + p(CaH) + p(CrH) 
!                  + p(FeH) + 2p(CH_2) + 2p(NH_2) + 2p(H_2O) + 2p(PH_2) 
!                  + 2p(SH_2) + p(HCO) + p(MgOH) + p(AlOH) + p(CaOH)
!     P(He) = p(He) + p(He+)
!     P(C)  = p(C) + p(C+) + p(CO) + p(CH) + 2p(CH_2) + p(HCO) + p(CN)
!                  + p(CS) + p(CO_2)
!     P(N)  = p(N) + p(N+) + 2p(N_2) + p(NH) + p(NH_2) + p(CN) + p(NO) 
!                  + p(SiO) + p(PN) + p(SN) 
!     P(O)  = p(O) + p(O+) + p(CO) + p(OH) + p(H_2O) + p(HCO) + p(MgOH) 
!                  + p(AlOH) + p(CaOH) + 2p(CO_2) + p(NO) + 2p(O_2)
!                  + p(MgO) + p(SiO) + p(PO) + p(SO) + p(TiO) + p(VO) + p(FeO)
!     P(Mg) = p(Mg) + p(Mg+) + p(MgH) + p(MGOH) + p(MgO)
!     P(Al) = p(Al) + p(Al+) + p(AlH) + p(AlOH) 
!     P(Si) = p(Si) + p(Si+) + p(SiH) + p(SiO) + p(SiS)
!     P(P)  = p(P) + p(P+) + p(PH) + p(PH_2) + p(PN) + p(PO)
!     P(S)  = p(S) + p(S+) + p(SH) + p(SH_2) + p(SC) + p(SN) + P(SO) + p(SiS)
!     P(Cl) = p(Cl) + p(Cl+) + p(ClH)                         
!     P(Ca) = p(Ca) + P(Ca+) + p(CaH) + p(CaOH)               
!     P(Ti) = p(Ti) + p(Ti+) + p(TiO)                         
!     P(V)  = p(V) + p(V+) + p(VO)                            
!     P(Cr) = p(Cr) + p(Cr+) + p(CrH)                         
!     p(Fe) = p(Fe) + p(Fe+) + p(FeH) + p(FeO)                
