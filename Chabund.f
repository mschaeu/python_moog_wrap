      ...
      subroutine chabund
!*****************************************************************************
!  this routine changes the input abundances according to the users request
!  and the re-runs the synthesis with the new abundances OVERWRITING the
!  original output files, but using the same model, linelist, etc
!*****************************************************************************
...
!*****set the local temporary parameters to what they were for the last synth
      newnumiso = numiso
...
!*****SPECIAL CASE
!     for the "binary" driver, present a limited set of options; only
!     the abundances of the elements already chosen for variation in
!     the parameter file can be altered
...
!*****for the "synth" driver, present the user with the current abundance
!     alterations, and the options; find out what is desired
...
!*****for "synth", change elemental abundances
      if     (choice .eq. 'c') then
...
!*****for "synth", change isotopic factors
...
!*****for "synth", change the number of syntheses
...
!*****for "synth", exit the routine without changing anything
      elseif (choice .eq. 'x') then
         return


!*****for "synth", make the proposed alterations permanent; then
!     return to the calling routine, which allegedly will
!     redo the syntheses.
...
!*****loop back and print out the main menu again
