      ...
      subroutine eqlib                                                  
!******************************************************************************
!     This routine solves the equations of molecular dissociative           
!     equilibrium. the equations can include neutral and ionized molecules  
!     and atoms 
!******************************************************************************
...
!*****either read in the dissociation data for a molecular species
      do jmol=1,nmol                                                   
...
!*****or read the ionization data for an atomic species
         else
...
!*****set up the molecular equilibrium array. each element of array         
!*****'ident' contains the identifier (subscript of an element of           
!*****array 'amol') of an ion or a molecule. the neutral atom is always     
!*****understood, but is not explicitly contained in 'ident'.               
      nmax = 1                                                          
      do jmol=1,nmol                                                  
         atom = amol(jmol)                                              
...
!*****now begin the loop that goes through all the atmosphere tau layers
      do 21 kev=1,ntau                                                  


!*****calculate *xfic* and make a first guess at *xatom*                    
      i = ntau + 1 - kev                                           
      lev = i
      tk = 1.38054d-16*t(i)                                             
      do k=1,neq                                                      
...
!*****Here is some information about the equilibrium constants.
!*****The equilibrium constants, Kp, are defined as follows:
!
!
!                 P(A)*P(B)     N(A)*N(B)
!       Kp(AB)  = --------- = kT--------- =
!                  P(AB)         N(AB)
!
!             2*pi*kT 3/2    M(A)*M(B) 3/2   Q(A)*Q(B)
!       = kT*(-------)    * (---------)    * --------- * exp(-D(AB)/kT)
!               h^2            M(AB)           Q(AB)
!
! 
!*****MOOG uses: n(AB)/n(A)n(B) = kt/Kp
! 
!        Kp - dissociation constant, Q - partition functions, M - masses
!        P - partial pressures, N - number densities, T - temperature,
!        D - dissociation energy, h - plank constant. Remember to use
!        masses in grams (1 amu = 1.660540E-24 g) and energy in ergs
!        (1 eV = 1.60219E-12 ergs). Also, k = 1.38066E-16 erg/K,
!        h = 6.626076E-27 erg s, and pi = 3.1415926536.
27    do jmol=1,nmol                                                  
         atom = amol(jmol)                                              
         if (atom .ge. 100.) then
...
!*****compute matrix *!*, which is the derivative of each equation with     
...
!*****calculate array 'xcorr', the change in 'xatom'. array 'xcorr' is      
!*****'deltax' multiplied by the inverse of 'c'                            
...
!*****print out atomic and molecular partial pressures. *xamol* is the      
!*****number density for each neutral atom                                  
      do k=1,neq                                                     
         xamol(k,i) = xatom(k)                                          
