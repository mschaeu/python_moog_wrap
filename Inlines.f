...
!******************************************************************************
!     This subroutine reads in the line data
!******************************************************************************
...
!*****decide if certain element abundances need to be modified.
...
!*****output information about the isotopi! ratios
      if (numiso .gt. 0) then
!         write (nf1out,1014)
         do i=1,numiso
            iiso = isotope(i)
...
!*****Inititalize strong line printing
!     if 'printstrong' gt 0 then the strong lines have 
!     been printed
      printstrong = -1
...
!*****read in the strong lines if needed
302   nstrong = 0
...
!*****append the strong lines here if necessary
      if (dostrong .gt. 0) then
         do k=1,nstrong
...
!*****here groups of lines for blended features are defined
...
!*****here excitation potentials are changed from cm^-1 to eV, if needed
      do j=1,nlines+nstrong
         if (e(j,1) .gt. 50.) then
...
!*****here log(gf) values are turned into gf values, if needed
      do j=1,nlines+nstrong
...
!*****turn log(RW) values and EW values in mA into EW values in A.  Stuff
!     duplicate EW values of the first line of a blend into all blend members.
      do j=1,nlines+nstrong
...
!*****here some parameters for the lines are assigned or calculated; 
!     there is a block of statements for moleculer lines, 
!     and a different one for atomic lines
      do j=1,nlines+nstrong
         iatom = atom1(j)
         atom10 = 10.*atom1(j)
...
!*****here are the calculations specific to molecular lines
...
!*****use an internal dissociation energy for molecules if the user
!     does not read one in
            if (d0(j) .eq. 0.) then
               do k=1,110
...
!*****here are the calculations specific to atomic lines
         else
            if (atom10-int(atom10) .le. 0.0) then
               amass(j) = xam(iatom)
            else 
...
!****prepare to get another chunk of line data 
