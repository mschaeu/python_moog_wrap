...
!******************************************************************************
!     This routine calculates the quantity 'kapnu0' for each line and
!     each tau; this is the line opacity at line center.  When kapnu0 is
!     multiplied by the voigt function, one gets the line opacity at an
!     arbitrary wavelength.  Then, the Doppler factor 'dopp' and damping
!     parameter 'a' are also computed ('a' is done in a call to another
!     routine
!******************************************************************************
...
!*****load in data for damping factors to be computed from the data
!     of Barklem, if desired
      if (numpass.eq.1 .and. dampingopt.eq.1) call gammabark


!*****Locate the atmosphere level where taulam is near 0.5
...
!*****if reading in a new line list, do all lines (numpass=1);
!     if iterating "abfind" on a species affected by mol. eq.,
!     do just that species (numpass=2); if doing a fake line, or
!     maybe one line for a special purpose, do just the first line 
!     in the "list" (numpass=3)
...
!*****now make the calculations:  set up some parameters
      do j=j1,j2
...
!*****compute damping parameters in a separate routine
            call damping (j)
         endif


!*****either: compute lower state number densities for atomic lines;
!     q21 is the ion/neutral ratio, etc., and q is the ratio of the total
!     to the species of interest; do the Saha equation first, then
...
!*****or: compute lower state number densities for molecular lines
         else
...
!*****finally, compute line opacities at line centers
...
!*****output regular line information, and strong line information
!     if appropriate; exit routine normally
      if (numpass.eq.1 .or. numpass.eq.3) then
