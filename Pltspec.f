...
!******************************************************************************
!     This subroutine controls the plotting of the synthesized and
!     observed data
!******************************************************************************
...
!*****initialize some variables, or re-set them to old values
...
!*****make a special "choice" for grid syntheses, which go directly to
...
!*****go through the option list; the routine may be exited at this point
      istat = ivcleof(4,1)
1     if (choice .eq. 'q') return


!*****or a default plot may be made upon entering the routine
      if (choice.eq.'1' .or. choice.eq.'g') then
!*****make a cross correlation to line up synthetic and observed spectra
!*****in velocity (wavelength) space; the user can turn this off/on
!*****not working for you
         call smooth (-1,ncall)
!----------------------------------------------------------------------------
...
!*****or the synthetic spectra may be resmoothed; if a problem develops in
!     a user-specified parameter (like a Gaussian FWHM that is too large),
!     then output a warning and let user decide what to do next
      if (choice .eq. 's') then
...
!*****or the observations may be rescaled
      if (choice .eq. 'r') then
...
!*****or the observations may be shifted by an additive constant
      if (choice .eq. 'a') then
...
!*****or the observations may be shifted by a constant wavelength
      if (choice .eq. 'w') then
...
!*****or the observations may be shifted by a constant velocity
      if (choice .eq. 'v') then
...
!*****or the plot boundaries may be changed
      if (choice .eq. 'c') then
...
!*****or the cross hairs can be used to zoom in on a part of the plot
      if (choice .eq. 'z') then
...
!*****or cursor position can be returned
      if (choice .eq. 'p') then
...
!*****or the title of the model can be changed
      if (choice .eq. 't') then
...
!*****or the spectra can be replotted, with a separate plot showing the
!     observed/synthtic spectrum differences
      if (choice .eq. 'd') then
         deviations = 1
         whichwin = '2of2'
      endif
...
!*****or the plot boundaries may be reset to the original values;
!     this is a basic starting over plot
      if (choice .eq. 'o') then
         xlo = start
         xhi = sstop
         if (iunits .eq. 1) then
...
!*****or the plot can simply be redone
      if (choice .eq. 'm') then
         go to 90
      endif


!*****now either here make a hardcopy plot
      if (choice .eq. 'h') then
...
!*****or write the plot to a postscript file
...
!*****or return to the calling routine in order to change abundances
      if (choice .eq. 'n') then
         call plotremember (3)
         return
      endif
...
!*****or add an additional uniform amount of flux, expressed in terms of
!     the current continuum flux; this only is approximately physically
!     correct if spectrograph smoothing is negligible compared to other
!     smoothing
      if (choice .eq. 'l') then
         write (array,*) &
...
!*****or if total confusion has happened in the plotting, reset all parameters
!     to their original values, and replot
      if (choice .eq. 'u') then
         call plotremember (2)
         ncall = 1
         go to 3
...
!*****finally, print the option table
...
!*****reprint the option table if the choice is not understood
!     or take action on the choice
