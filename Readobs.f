...
!******************************************************************************
!     this routine reads in observed spectra.  the format is indicated by
!     the value of "specfileopt"
!******************************************************************************
...
!*****first set some initial parameters
      write (obsitle,1021)
      iend = 0
      naxis = 1
      naxis1 = 0
...
!*****branch to the appropriate data file type
      go to (100,200,300,400,500), specfileopt


!*****here is a pure FITS file type. Reading is done with ordinary
!     FORTRAN read statements
!     first get the header records and search for key parameters
100   irec = 1
...
!     next read the flux array from the file
      nrec = lount/nblock
      ipt = 0
...
!     now adjust the continuum, if desired
      do j=1,lount
         yobs(j) = yobs(j)*contnorm
      enddo


!     now fill in the wavelength array
      do j=1,lount
...
!     or if trouble, give error messages, and abort
...
!*****this is a dummy for future file use
200   return
...
!*****here is a MONGO-style input array
500   rewind nfobs
