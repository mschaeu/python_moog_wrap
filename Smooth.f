...
!******************************************************************************
!     This subroutine prepares synthesis data for plotting by smoothing
...
!*****on entering, figure out what kind of smoothing is desired, unless
!     the default smoothing options have been set for first pass
      if (line .gt. 0) then
2        write (array,1007)
...
!     if a user-specified variable Gaussian smoothing over the spectral range
!     is called for, option 'p', branch to a different routine
      if (smtype .eq. 'p') then
...
!*****rewind the unsmoothed and smoothed spectrum output
!     files, and get the synthesis range parameters from the 'dump' file
      rewind nf2out
      rewind nf3out
      do i=1,20
...
!*****branch to the desired smoothing function
...
!*****compute a macroturbulent smoothing function (uses subroutine vmacro)
...
!*****compute a Gaussian smoothing function
...
!*****compute a Lorenzian smoothing function
...
!*****finally there is a big loop that will read in the raw synthetic spectra,
!*****beginning with grabbing the information that appears before the array
!*****spectrum depth array,  Note that after the depth array is input, the
!*****code will flip to a flux scale
      rewind nf2out
      do nsyn=1,100


!*****here is the reading/grabbing of stuff preceding the depth array:
         naboff = 80*(nsyn-1)
         nabunds = 0
         nisos = 0
         do i=1,20
...
!*****here is the actual reading of the depth array
         read (nf2out,1003,end=2000) (ysyn(i),i=1,kount)
         do i=1,kount
            ysyn(i) = 1.0 - ysyn(i)
         enddo


!*****here a veiling addition can be added in
         if (addflux .gt. 0.0) then
             do i=1,kount
                 ysyn(i) = (ysyn(i) + addflux)/(1.0+addflux)
             enddo
         endif


!*****apply the rotational broadening if desired
         if (rotateflag .eq. 't') then
            min = jdelrot + 1
            max = kount - jdelrot
            do i=1,jdelrot
            ...
            do i=min,max
               zsyn(i) = prot0*ysyn(i)
               do j=1,jdelrot
                  zsyn(i) = zsyn(i) + prot(j)*(ysyn(i-j) + ysyn(i+j))
               enddo
               zsyn(i) = zsyn(i)/powerrot
            enddo
            do i=1,kount
               ysyn(i) = zsyn(i)
            enddo
...
!*****apply the macroturbulent broadening if desired
         if (macroflag .eq. 't') then
            min = jdelmac + 1
            max = kount - jdelmac
            do i=1,jdelmac
            ...
            do i=min,max
               zsyn(i) = ysyn(i)
               do j=1,jdelmac
                  zsyn(i) = zsyn(i) + pmac(j)*(ysyn(i-j) + ysyn(i+j))
               enddo
               zsyn(i) = zsyn(i)/powermac
            enddo
            do i=1,kount
               ysyn(i) = zsyn(i)
            enddo
...
!*****apply the Gaussian or Lorenzian smoothing if desired (this
!     is an either/or situation; only one of these can apply.
         if (gaussflag .eq. 't' .or. lorenflag .eq. 't') then
            ...
            do i=min,max
               zsyn(i) = ysyn(i)
               do j=1,jdel
                  zsyn(i) = zsyn(i) + p(j)*(ysyn(i-j) + ysyn(i+j))
               enddo
               zsyn(i) = zsyn(i)/power
            enddo
            do i=1,kount
...
!*****move the smoothed spectrum (or unsmoothed, if nothing has
!     been done to the y-array) into the appropriate array
         do i = 1,kount
            chunk(i,nsyn) = ysyn(i)
         enddo
   

!*****compute the wavelength array; must be done for each synthetic
!     spectrum because of the way the equivalences were set up
         if     (iunits .eq. 1) then
            do i=1,kount
...
!*****dump the smoothed spectrum in a MONGO-style set of
!     (wavelength,flux) point pairs
...
!*****exit the routine normally
2000  nsyn = nsyn - 1
      return


!*****a problem has developed in user-specified parameter (like a Gaussian
!     FWHM being too big); print out a warning and let user decide what
!     to do next
