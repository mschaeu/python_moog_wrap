      ...
      subroutine specplot
!******************************************************************************
!     This routine produces MONGO plots of the syntheses.
!******************************************************************************
...
!*****for grid syntheses, dump out relevant information to a file
      if (choice .eq. 'g') then
...
!*****write smoothing information at the top of the plot
      call sm_lweight (2.2)
      call sm_expand (0.7)
...
!*****draw and label the box for the spectra
      call defcolor (1)
...
!*****this section of code is executed only if a deviations plot is desired;
!     find the starting and stopping points in the arrays for the deviations
      open (29,file='deviations')
...
!  compute the deviations; linear interpolation in the wavelength array
!  of the synthetic spectra is considered sufficient
165   do j=1,nsyn
         lpoint = lim1syn
         devsigma = 0.
...
170         if (xsyn(lpoint+1) .lt. xobs(i)) then
         ...
         itot = lim4obs - lim3obs + 1
         write (29,*) 'synthesis number, total points:', j, itot
         do i=1,itot
            if (abs(dev(i)) .gt. 1) then
               devwrite = 99.9999
            else
               devwrite = dev(i)
            endif
            write (29,3333) xobs(i), devwrite
         enddo
3333     format (f12.3, f12.4)
         
      
!  from first set of deviations, define the plot limits, draw and label box
         if (j .eq. 1) then
            yup = -1000.
            ydown = +1000.
            do i=lim3obs,lim4obs
...
!  reset the spectrum plot boundaries before exiting
      if(xlo .lt. xhi) then
         call sm_limits (xlo,xhi,ylo,yhi)
         iflip = 0
      else
