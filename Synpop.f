      ...
      subroutine synpop
!******************************************************************************
!     Synthetic spectrum routine for stellar populations; usually meant for
...
!*****FIRST PASS:  For each model, compute a raw synthetic spectrum;
!*****the starting do/if loops are for isotopic things only
      xhyd = 10.0**xsolar(1)
      do mmod=1,modtot
         if (numiso .gt. 0) then
            if (nisos .gt. 0) then
...
!*****change the "default" abundances to be the ones read from the table file
         do i=1,nabs
...
!*****clear the mean spectrum array and the information array
      do i=1,5
         do j=1,10000
            tempspec(i,j) = 0.
         enddo
...
!*****open the file where the mean raw spectrum will be written
      nf9out = 23
      lscreen = 10
...
!*****read back the syntheses, compute the weighted average
      do mmod=1,modtot
         newunit = 26
...
!     write the average spectrum back to disk.
      line(1:7) = 'MODEL: '
...
!*****if needed, loop back with abundance changes
         if (choice .eq. 'n') then
            call chabund
