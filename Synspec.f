      ...
      subroutine synspec
!******************************************************************************
!     This routine does synthetic spectra
!******************************************************************************
...
!*****now step in wavelength as the spectrum is computed
      kount = nint((sstop - start + (step/4.0) )/step) + 1
      num = 0
      wavl = 0.


!*****first calculate or recalculated continuum quantities at the
!     spectrum wavelength, if needed
      do n=1,kount
         num = num + 1
...
!*****find the appropriate set of lines for this wavelength, reading
!     in a new set if this is the initial depth calculation or if
!     needed because the line list end has been reached
         if (mode .eq. 3) then
20          call linlimit
...
!*****compute a spectrum depth at this point; if there are no absorption
!     lines in the interval then just set the depth to zero without
!     extensive line calculations
