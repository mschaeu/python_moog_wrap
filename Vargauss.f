...
!     the data with a Gaussian whose FWHM has been specified by the user
!     at various wavelengths along the spectrum
!******************************************************************************
...
!*****on entering, rewind the unsmoothed and smoothed spectrum output
...
!*****the first time through, read in the Gaussian FWHM array
      if (istart .eq. 0) then
         istart = 1
         nfsmooth = 35
...
!*****now read in the raw spectrum and flip to a depth scale
45    noff = 80*(nsyn-1)
...
!*****go through the spectrum wavelengths step by step; the
!     Gaussian smoothing will need to be different at each step
      i = 0
      oldhalf = 0.
25    i = i + 1
...
!*****interpolate linearly in the FWHM array to get the appropriate value
!     for the current wavelength step
...
!*****compute the Gaussian smoothing function, if needed
...
!*****if no smoothing, just equate the smoothed to the unsmoothed point
15    zsyn(i) = ysyn(i)
      go to 25

  
!*****otherwise smooth the spectrum
50    if (i.lt.min .or. i.gt.max) then
         zsyn(i) = ysyn(i)
      else
         zsyn(i) = p0*ysyn(i)
         do k=1,jdel
            zsyn(i) = zsyn(i) + p(k)*(ysyn(i-k) + ysyn(i+k))
         enddo
         zsyn(i) = zsyn(i)/power
      endif
      go to 25
...
!*****copy the smoothed spectrum to the appropriate array
90    do i=1,kount
         chunk(i,nsyn) = zsyn(i)
      enddo
...
!     (wavelength,flux) point pairs
      ...
      if (xsyn(1) .le. 100.0) then
         write (nf3out,1009) (xsyn(i),zsyn(i),i=1,kount)
      else 
         write (nf3out,1008) (xsyn(i),zsyn(i),i=1,kount)
      endif
      nsyn = nsyn + 1
      go to 45
